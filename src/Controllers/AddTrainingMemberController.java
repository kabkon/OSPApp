 ...
 */
public class AddTrainingMemberController implements Initializable {
    ...
    public void actionAddTraining(ActionEvent actionEvent) {
        ...
        if(checkFill()){
            ...
            try {
                ...
                AddMemberController.dataTrainings.add(new TableTrainings(AddMemberController.id,tfTrainingName.getText(),String.valueOf(dpTrainingDate.getValue()),tfTrainingPlace.getText()));

                Stage stage = (Stage) tfTrainingName.getScene().getWindow();
                insert.close();
                createScene.setPrevStage(stage);
                createScene.swapScene("/Windows/AddTrainingMemberWindow.fxml","Dodaj szkolenie");
