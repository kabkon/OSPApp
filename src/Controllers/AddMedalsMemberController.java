 ...
 */
public class AddMedalsMemberController implements Initializable {
    ...
    public void actionAddMedalMember(ActionEvent actionEvent) {
        ...
        if(checkFill()){
            ...
            try {
                ...
                AddMemberController.dataMedals.add(new TableMedals(AddMemberController.id,tfMedalsName.getText(),String.valueOf(dpMedalsDate.getValue()),taMedalsInfo.getText()));

                Stage stage = (Stage) tfMedalsName.getScene().getWindow();
                insert.close();
                createScene.setPrevStage(stage);
                createScene.swapScene("/Windows/AddMedalsMemberWindow.fxml","Dodaj odznaczenie");
